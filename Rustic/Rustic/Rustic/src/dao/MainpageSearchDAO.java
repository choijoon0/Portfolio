...
public class MainpageSearchDAO {
	...
	public ArrayList MypageList(int custnum) throws Exception
	{
		...
		pstmt = conn.prepareStatement("select RESERVNUM, RESERVDATE, RESERVPAYMENTMETHOD, RESERVCHECKIN, RESREVCHECKOUT from RESERVATION where reservcustnum = ?");
	...
	// 확정 메소드
		...
		public void reservnotcheck(String reservnum) throws Exception
		{
			...
			String sql = "delete reservation where reservnum = ?";
			//4. 전송객체 생성
			PreparedStatement pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, reservnum);
